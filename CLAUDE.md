...
A formal ontology of **constructed beings** in fiction — entities that are *made*, not *born*. 536 YAML entries coded against a seven-property analytical schema, spanning Homer (~750 BCE) through 2025. The dataset accompanies the essay "Tears in Rain" at tearsinrain.ai.

## Repository structure

```
data/beings/          536 YAML entry files (the corpus)
...
Current dataset release: **v3.1** (schema stays at **v3.0** in `schema/cb-schema.yaml`). v3.0 added the `metadata.origin` axis and refined the inclusion rule for born-then-modified beings; v3.1 is a review-and-expansion release (522 → 536 entries, no schema changes). See `docs/boundary_cases.md` for the inclusion criterion and CHANGELOG.md for history.
