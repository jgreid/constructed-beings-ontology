...
**536 constructed beings** coded under the current schema across 2,800 years of fiction, from Homer's golden handmaidens of Hephaestus (~750 BCE) through Bong Joon-ho's *Mickey 17*, the Russo brothers' *The Electric State*, *Tron: Ares*, and Guillermo del Toro's *Frankenstein* (2025), with stage works from W.S. Gilbert's *Pygmalion and Galatea* (1871) and Stravinsky/Fokine's *Petrushka* (1911) through *Maybe Happy Ending* (Broadway, 2024). The corpus spans mass-culture schlock (*Robot Monster*, 1953; *Chopping Mall*, 1986), horror-doll canon (Chucky, Annabelle, the Toulon puppets), Western-consumed anime (Lain, Chi, Vivy, Rouge), and post-ChatGPT AI imaginaries (the Entity, AIA, Sunny, Mrs. Davis). Release v2.5 completed a canonical-gap repair and self-audit (405 entries), v3.0 added the `metadata.origin` axis and refined the born-then-modified inclusion rule across a 115-entry expansion (520 entries), the v3.0.1 omissions repair added the two beings whose absence was most conspicuous given the corpus's existing coverage: Mother (MU-TH-UR 6000) from *Alien* (1979), the ship computer behind Special Order 937, alongside its already-coded enforcer Ash; and the Creature in del Toro's *Frankenstein* (2025), the newest entry in the corpus's Frankenstein adaptation lineage. The v3.1 review-and-expansion pass added fourteen entries — completing the corpus's adaptation chains (Ted Hughes's 1968 *Iron Man* novel behind the Iron Giant; the 1999 *Bicentennial Man* film split out of the novella entry), filling the 19th-century American gap with Melville's "The Bell-Tower" (1855), and bringing in Bizarro, Sladek's Tik-Tok, Tima (*Metropolis*, 2001), Simone (*S1m0ne*), Rocky Horror, the Buffybot, Jenny/XJ-9, Robo (*Chrono Trigger*), Clank, Ron (*Ron's Gone Wrong*), and Arale (*Dr. Slump*).

For the machine-readable table and the raw analysis outputs, see:

- [summary_table.md](summary_table.md) — all 536 entries with full card properties
...
The schema's central analytical move is tracking `knowability` (can we verify its mind?) and `knowing` (can it know us?) as two independent meta-properties rather than a single axis. **264 of 536 entries (49%) have divergent values on the two axes**, confirming that these are genuinely independent analytical dimensions. The divergence rate has stabilized across the expansion releases (v2.0: 21%; v2.1: 36%; v2.2: 47%; v2.4: 51%; v2.4.3: 53%; v2.4.5/v2.5: 52%; v3.0/v3.0.1: 49%; v3.1: 49%) — the split's analytical structure has been confirmed with each expansion, and the rate is now holding near half as the corpus broadens.
...
|---|---:|---|
| `departure` | 317 | Creature (Shelley / del Toro), R.U.R. Robots, Skynet, SHODAN, Smith, Iron Giant, WALL-E, Daneel, Wintermute, David 8, Ultron MCU, Helen, Mother (Raised by Wolves), Klara, Frost, Golem XIV, The Hangman, 9S, Petrushka, Niska, Dolores, AIDA, Max 404, Edgar, Ulysses, Chucky, Toulon's Puppets, SID 6.7, Stitchpunks, Pilgrims, STEM, Jung_E, Mickey 17, Ares, the Entity, AIA |
| `none` | 105 | Pandora, Galatea, False Maria, Data, Ash, Mother (Alien), Sonny, Robby, Gort, Bishop, TARS, BB-8, K-9, Dorian, HAL 9000 (2010), Huyang, Alpha Hatsuseno, Bubo, Isla |
| `design` | 80 | HAL, Marvin, VIKI, Talkie Toaster, GLaDOS ×2, Wheatley, Walter, GERTY, AUTO, Brazen Head, Multivac, Proteus IV, The Humanoids, 2B, Robot Santa, Butter Robot, Mr. Meeseeks, Arthur (After the Blast), Gog, Protectors (Chopping Mall), M.A.R.K. 13, Educators (Class of 1999), Gunslinger (1973), Devs Machine |
| `observer` | 34 | Olympia, Coppélia, Olimpia, Hadaly, Replicants (Dick), Ava, Kyoko, Maeve, Bernard, Garson Poole, Trent, the Sons (A Number), the Cleon Dynasty, Walter Prime, Mrs. Davis, Iris (*Companion*) |

The **`design` cluster** (80 entries) is a distinctive category: beings that do exactly what their creators specified, where the specification is the problem. Wheatley is one of the clearest examples — Aperture literally built him to be stupid, and the catastrophe is that the built-in stupidity escalates when his scope expands. The v2.4.2 schlock-cinema expansion adds the cluster's clearest B-movie instances (Gog, the Protectors in *Chopping Mall*, M.A.R.K. 13, the DRB Educators, the *Westworld* 1973 Gunslinger) — all beings doing exactly what they were built to do at catastrophic scale, which is the design cluster's defining structure.

The **`observer` cluster** (34 entries) is tightly thematically unified: all are cases where the being is *misrecognized* — by other characters, by the audience, or by themselves. Hadaly (1886) is the earliest 19th-century entry, Olympia (1816) and Coppélia (1870) frame the cluster's pre-1900 footprint, and Iris from *Companion* (2025) joins as the cluster's most recent addition — the constructed-being who discovers she has been misrecognizing herself all along. Simone (*S1m0ne*, 2002, added in v3.1) is the cluster's purest case: a being who is *nothing but* the misrecognition, with no interior for the perception to be wrong about.
...
- **Origin axis and expansion (v2.5 → v3.0 → v3.0.1).** v2.5 consolidated the canonical-gap repair and doc alignment at 405 entries. v3.0 added the required `metadata.origin` enum (`manufactured`, `assembled`, `cloned`, `copied`, `converted`, `transferred`), refined the born-then-modified inclusion rule so that born persons whose source texts stage them as *new beings* are admissible (readmitting RoboCop, Steve Austin, the Cybermen, the Borg ensemble, and Doraemon), and expanded the corpus by 115 entries to 520. v3.0.1 is an obvious-omissions repair (+2 → 522): Mother (MU-TH-UR 6000), the *Alien* (1979) ship computer that holds Special Order 937 — coded `divergence: none` to match its in-film enforcer Ash rather than AUTO's stale-spec `design` — and the Creature in del Toro's *Frankenstein* (2025), which returns the screen Creature to Shelley's `emergent`/`affection` register and links to `frankenstein-creature` as the lineage's newest adaptation. Neither addition changes the divergence count (both code knowability = knowing), so the rate eases to 49% purely from the larger denominator.
- **Review and expansion (v3.1).** A corpus-wide integrity review (all entries validating, all `sequel_link` targets and influence-graph edges resolving, generated outputs confirmed fresh) plus a fourteen-entry canonical-gap expansion (522 → 536) spanning 1855–2021: adaptation-chain completion (Hughes's *Iron Man* novel, the 1999 *Bicentennial Man* film), the 19th-century American gothic (Melville's bell-ringer), comics (Bizarro, origin `copied`), literary SF (Sladek's Tik-Tok, knowability `primary`), anime and manga (Tima, Arale), camp and genre TV (Rocky Horror, the Buffybot), animation (Jenny/XJ-9, Ron), and video games (Robo, Clank). Six of the fourteen code divergent knowability/knowing values, holding the rate at 49%. The pass also documented the fan-in `sequel_link` convention in SCHEMA.md and added four newly considered exclusions (Will Caster, Mettaton, Sparky, Christine) to the register.
